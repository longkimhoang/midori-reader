    ...
    guard result == "ok" else {
      throw DecodingError.dataCorrupted(
        DecodingError.Context(
          codingPath: container.codingPath, debugDescription: "result must be ok"))
    }

    let tokenContainer = try container.nestedContainer(
      keyedBy: TokenCodingKeys.self, forKey: .token)
